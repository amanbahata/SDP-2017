    ...
    for (line <- lines) {
      ...
      if (fields.length > 0) {
        ...
        fields(1) match {
          ...
          case BNZ =>
            program = program :+ BnzInstruction(fields(0), fields(2).toInt, fields(3).toInt, fields(4).toInt)
          case MUL =>
          ...
          case OUT =>
            program = program :+ OutInstruction(fields(0), fields(2).toInt, fields(3).toInt, fields(4).toInt)
          case x =>
